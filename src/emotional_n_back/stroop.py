...
class AlternatingStroopGame:
    """
    A sentiment-based Stroop game that alternates between modalities.

    In each trial, a face and an audio clip with random sentiments are presented.
    The task alternates between identifying the sentiment of the audio and the face.

    - On AUDIO trials (even trial numbers), the user must identify the sentiment
      of the AUDIO clip, ignoring the face.
    - On VISUAL trials (odd trial numbers), the user must identify the sentiment
      of the FACE, ignoring the audio. A yellow border will highlight the face.
    ...
    def __init__(
        ...
        *,
        length: int = 30,
        seed: Optional[int] = None,
        intro_delay_ms: int = 500,  # Time between first and second stimulus
        response_window_ms: int = 2000,  # Time to answer after second stimulus
        feedback_ms: int = 500,
        isi_ms: int = 300,
        window_size=(900, 650),
    ):
        if seed is not None:
            random.seed(seed)
        self.length = length
        self.intro_delay_ms = intro_delay_ms
        self.response_window_ms = response_window_ms
        self.feedback_ms = feedback_ms
        self.isi_ms = isi_ms
        ...
        pygame.mixer.init(frequency=44100, size=-16, channels=2, buffer=512)
        self.screen = pygame.display.set_mode(window_size)
        pygame.display.set_caption("Alternating Sentiment Stroop Game")
        self.clock = pygame.time.Clock()
        self.font_big = pygame.font.SysFont(None, 48)
        self.font_small = pygame.font.SysFont(None, 24)
    ...
    def _draw_header(self):
        ...
        if self.is_audio_trial:
            tip_text = "What is the sentiment of the audio? (1: Pos, 2: Neu, 3: Neg)"
        else:
            tip_text = "What is the sentiment of the face? (1: Pos, 2: Neu, 3: Neg)"
    ...
    def _draw_stimulus_box(
        self, image_surface: Optional[pygame.Surface] = None, highlight: bool = False
    ):
        pygame.draw.rect(
            self.screen, (60, 60, 65), self.stimulus_rect, border_radius=12
        )
    ...
    def run(self):
        ...
        while running and self.trial_num < self.length:
            # --- 1. Prepare Trial ---
            self.is_audio_trial = self.trial_num % 2 == 0
            visual_sentiment = random.choice(self.sentiments)
            audio_sentiment = random.choice(self.sentiments)
            ...
            # --- 2. Intro Phase ---
            # On visual trials (is_audio_trial=False), the audio is the distractor and comes first.
            # On audio trials (is_audio_trial=True), the visual is the distractor and comes first.
            if not self.is_audio_trial:
                audio_sound.play()

            intro_t0 = pygame.time.get_ticks()
            while pygame.time.get_ticks() - intro_t0 < self.intro_delay_ms:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT or (
                        event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE
                    ):
                ...
                # Show visual distractor on audio trials; show blank on visual trials
                intro_image = image_surface if self.is_audio_trial else None
                self._draw_stimulus_box(
                    intro_image, highlight=not self.is_audio_trial
                )
                self._draw_scorebar()
                pygame.display.flip()
                self.clock.tick(120)
            ...
            # --- 3. Response Phase ---
            # The target stimulus is presented now.
            if self.is_audio_trial:
                audio_sound.play()  # Play target audio
            ...
            while pygame.time.get_ticks() - response_t0 < self.response_window_ms:
                    ...
                    if event.type == pygame.QUIT:
                        ...
                        if event.key == pygame.K_ESCAPE:
                            ...
                            target_sentiment = (
                                audio_sentiment
                                if self.is_audio_trial
                                else visual_sentiment
                            )
                            correct_answer = user_sentiment == target_sentiment
                ...
                self.screen.fill((20, 22, 26))
                self._draw_header()
                # During response phase, the visual is always shown.
                self._draw_stimulus_box(
                    image_surface, highlight=not self.is_audio_trial
                )
                self._draw_scorebar()
                pygame.display.flip()
                self.clock.tick(120)
